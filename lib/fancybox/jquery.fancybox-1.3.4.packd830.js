...
;(function(b) {
    var m, t, u, f, D, j, E, n, z, A, q = 0, e = {}, o = [], p = 0, d = {}, l = [], G = null, v = new Image, J = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i, W = /[^\.]\.(swf)\s*$/i, K, L = 1, y = 0, s = "", r, i, h = false, B = b.extend(b("<div/>")[0], {
        prop: 0
    }), M = false && b.browser.version < 7 && !window.XMLHttpRequest, N = function() {
        t.hide();
        ...
        var a = U()
          ...
          , g = d.autoScale
          , k = d.padding * 2;
        c.width = d.width.toString().indexOf("%") > -1 ? parseInt(a[0] * parseFloat(d.width) / 100, 10) : d.width + k - 50;
        c.height = d.height.toString().indexOf("%") > -1 ? parseInt(a[1] * parseFloat(d.height) / 100, 10) : d.height + k - 50;
        if (g && (c.width > window.innerWidth || c.height > window.innerHeight) && (c.width > a[0] || c.height > a[1]))
            if (e.type == "image" || e.type == "swf") {
                g = d.width / d.height;
                if (c.width > a[0] && a[0] > 0) {
                    c.width = a[0];
                    c.height = parseInt((c.width - k) / g + k, 10)
                }
                if (c.height > a[1] && a[1] > 0) {
                    c.height = a[1];
	...
	c.top = ((window.innerHeight - c.height) / 2) - 20;
	...
	if (window.innerWidth < 800) {
	    if (c.width >= c.height || window.innerWidth < 450) {
		c.width += 20;
